...
from typing import Any, Awaitable, BinaryIO, Callable, ClassVar, Dict, Optional, TypeVar, Union
from unittest import IsolatedAsyncioTestCase, SkipTest

from tinode_grpc import pb
from aiofiles import open as aio_open

from karuha import Config, async_run, cancel_all_bots, get_bot, reset, try_add_bot
...
class MockServer(BaseServer, type="mock"):
    ...
    async def upload(self, path: Union[str, os.PathLike, BinaryIO], auth: str, **kwds: Any) -> Dict[str, Any]:
...
class AsyncBotTestCase(IsolatedAsyncioTestCase):
    ...
    def confirm_message(self, tid: Optional[str] = None, code: int = 200, **params: Any) -> str:
        ...
        self.bot._wait_list[tid].set_result(
            pb.ServerCtrl(id=tid, code=code, text=text, params={k: json.dumps(v).encode() for k, v in params.items()})
        )
        return tid
    ...
    async def assert_note_read(self, topic: str, seq_id: int, /) -> None:
        assert await self.get_bot_sent() == pb.ClientMsg(note_read=pb.ClientNote(topic=topic, seq_id=seq_id, what=pb.READ))
...
def new_test_message(content: bytes = b'"test"', *, topic: str = TEST_TOPIC, user_id: str = TEST_UID) -> Message:
    return Message.new(bot_mock, topic, user_id, 1, {}, content)


def new_test_command_message(content: bytes = b'"test"') -> CommandMessage:
    return CommandMessage.from_message(
        new_test_message(content), FunctionCommand("test", lambda: None), new_collection(), "test", []
    )
